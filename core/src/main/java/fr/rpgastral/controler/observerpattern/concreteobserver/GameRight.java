...
public class GameRight implements Observer{

	private RpgMain g;
	
	public GameRight(RpgMain game) {
		this.g = game;
	}
	
	@Override
	public void update(Event event) {
		if(event.compare(new Event("GameScreen", true, "RIGHT"))) {
			Boolean valid = true;
			...
			if(g.getGamescreen().getPlayer().Getx() +1 <= g.getTiledModel().getWidth() && valid) {
