...
public class GameUp implements Observer{

	private RpgMain g;
	
	public GameUp(RpgMain game) {
		this.g = game;
	}
	
	@Override
	public void update(Event event) {
		if(event.compare(new Event("GameScreen", true, "UP"))) {
			Boolean valid = true;
			...
			if(g.getGamescreen().getPlayer().Gety()+1 < g.getTiledModel().getHeight() && valid) {
