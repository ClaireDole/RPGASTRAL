...
import fr.rpgastral.controler.observerpattern.concreteobserver.Msg3;
		...
		if (this.c != null && choix) {
			...
			if (c instanceof Tenue) {
				this.font.Getfont1().draw(this.game.getBatch(),
						"Choix 1 : " + this.game.getGamescreen().getPlayer().getTenue().getName(), 50, 400);
				this.font.Getfont1().draw(this.game.getBatch(),
						this.game.getGamescreen().getPlayer().getTenue().getDescription(), 100, 350);
			...
			if(c instanceof Armes) {
				this.font.Getfont1().draw(this.game.getBatch(),
						"Choix 1 : " + this.game.getGamescreen().getPlayer().getMd().getName() +" "+ this.game.getGamescreen().getPlayer().getMg().getName(), 50, 500);
				this.font.Getfont1().draw(this.game.getBatch(),
						this.game.getGamescreen().getPlayer().getMd().getDescription(), 100, 450);
				this.font.Getfont1().draw(this.game.getBatch(),this.game.getGamescreen().getPlayer().getMg().getDescription(), 100, 400);
				if(((Armes) c).getMaindouble()) {
					...
					this.font.Getfont1().draw(this.game.getBatch(), "Choix 2 : remplacer l'arme " + this.game.getGamescreen().getPlayer().getMd().getName()+ " par "+ c.getName(), 50, 300);
					this.font.Getfont1().draw(this.game.getBatch(), "actuellement : " + this.game.getGamescreen().getPlayer().getMd().getDescription(), 100, 250);
					this.font.Getfont1().draw(this.game.getBatch(), "remplacer par : "+c.getDescription(), 100, 200);
					
					this.font.Getfont1().draw(this.game.getBatch(), "Choix 3 : remplacer l'arme " + this.game.getGamescreen().getPlayer().getMg().getName()+ " par "+ c.getName(), 50, 150);
					this.font.Getfont1().draw(this.game.getBatch(), "actuellement : " + this.game.getGamescreen().getPlayer().getMg().getDescription(), 100, 100);
		...
		if(c instanceof Armes) {
			if(!(((Armes) c).getMaindouble())) {
				if (Gdx.input.isKeyPressed(Input.Keys.NUM_3)) {
					Event event = new Event(this.game,"MsgScreen",true,"3",c);
					notify(event);
					this.dispose();
				}
			}
